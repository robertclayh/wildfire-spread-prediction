...
We frame next-day spread prediction as binary segmentation on $H\times W$ tiles. Three baselines anchor the study: (i) a per-pixel logistic regression classifier, (ii) a shallow CNN to measure gains from local spatial context, and (iii) a compact U-Net with a lightweight encoder (e.g., ResNet-18) for multiscale reasoning. The latter two models will later be paired with exponential moving average (EMA) Polyak weight averaging to stabilise validation metrics.
...
\subsubsection{Future Deep Baselines}
The shallow CNN baseline reuses the same inputs but introduces $3\times3$ convolutions without skip connections, while the compact U-Net employs an encoder–decoder with bilinear upsampling and skip links. Both models will be trained with combo losses (Dice + weighted BCE) and evaluated under EMA Polyak averaging to reduce variance. These deeper baselines provide context for measuring the lift delivered by spatial modeling beyond the logistic regression reference point.
...
The end-to-end pipeline now converts mNDWS TFRecords into \texttt{.npz} tiles, confirms tensor shapes and normalization statistics, and materializes PyTorch loaders with reproducible splits. Figure~\ref{fig:lr_training_curves} (placeholder) summarises the 50-epoch sanity run for the logistic regression baseline: binary cross-entropy steadily declines and the validation precision--recall area (AP) climbs to $0.214$ with a corresponding F1 peak of $0.167$ at a probability threshold of $0.23$. Evaluating the frozen model on the held-out test split produces AP $=0.198$ and F1 $=0.142$, with precision $0.311$ at recall $0.201$. Thresholded predictions concentrate along prior fire perimeters as illustrated in Figure~\ref{fig:lr_tile_example} (placeholder), though missed interior ignition pockets motivate spatial baselines. Confusion matrices in Figure~\ref{fig:lr_confusion} (placeholder) highlight the severe class imbalance: most errors are false positives in low-burn regions, while recall remains near $0.20$ at the calibrated operating point. Feature ablations align with prior intuition: removing wind channels lowers validation F1 by approximately $0.04$, while dropping the latent fuel embeddings causes a smaller $0.01$ decline. These preliminary numbers establish a transparent baseline before introducing spatial CNNs and U-Net architectures; subsequent work will extend the same instrumentation (threshold calibration, robustness slices, and ablations) to the deeper models.

\begin{figure}[t]
    \centering
    % Placeholder: replace with actual training curve image
    \includegraphics[width=0.95\linewidth]{figures/lr_training_curves.png}
    \caption{Logistic regression training loss and validation precision--recall metrics over 50 epochs.}
...
\begin{figure}[t]
    ...
    \caption{Example tile showing previous burn mask, thresholded logistic regression prediction, and next-day ground truth.}
...
\begin{figure}[t]
    ...
    \includegraphics[width=0.95\linewidth]{figures/lr_confusion_matrices_placeholder.png}
    \caption{Validation and test confusion matrices at the calibrated probability threshold ($\approx0.23$).}
