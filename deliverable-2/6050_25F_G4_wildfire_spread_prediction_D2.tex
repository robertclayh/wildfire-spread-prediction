...
\section{Method}
\subsection{Dataset}
We use the modified Next Day Wildfire Spread (mNDWS) dataset \cite{hulsey2024mndws}. It packages 500 m VIIRS tiles spanning the western contiguous United States between 2018 and 2023 with predefined train/validation/test splits and per-channel normalization statistics. Each sample includes dynamic weather aggregates (precipitation, temperature, wind speed and direction), vegetation and drought indicators (NDVI, ERC, CHILI), latent fuel embeddings (fuel1--3) derived from 30 m LANDFIRE fuels, topography (elevation), and impervious/water masks. Class imbalance is severe—roughly 3\% of pixels are positive—so all models will incorporate loss re-weighting or focal components. We treat prediction as binary segmentation (burn/no-burn) for day $t{+}1$.

\subsection{Baselines and Training Setup}
We frame the task as next-day binary segmentation on $H\times W$ tiles. Our baseline lineup is:
\begin{enumerate}
    \item Logistic regression trained per pixel on the tabular covariates with $L_{2}$ regularization.
    \item A shallow CNN without skip connections to establish the value of local spatial context.
    \item A compact U-Net using a lightweight encoder (e.g., ResNet-18) to capture multiscale spatial patterns while keeping parameter count tractable.
\end{enumerate}
Inputs consist of the standardized mNDWS channels listed above. For imbalance, we will experiment with focal plus Dice loss as well as weighted binary cross-entropy plus Dice. Early stopping will monitor validation F1. To aid interpretation we will perform feature-family ablations by removing wind, fuels, vegetation/drought, or topography channels and measuring F1 and IoU changes. Score calibration will be handled by tuning thresholds on validation precision–recall trade-offs and plotting reliability curves.

\subsection{Evaluation Protocol}
Model comparisons will emphasize task-specific metrics: micro-averaged F1, Intersection-over-Union (IoU), precision, recall, and precision–recall curves. We will document hyperparameters, report parameter counts, and track approximate training and inference times for each model. Robustness analyses will slice results by high-wind quartiles (\texttt{wind\_75}), impervious fraction (as a WUI proxy), and broad ecoregion or elevation bands when available. Each ablation will modify a single component at a time with shared random seeds and configuration tracking for reproducibility.

\section{Preliminary Experiments}
We completed TFRecord ingestion and confirmed tensor shapes, normalization statistics, and class imbalance (3\% positives) using a 50-tile sample. The logistic-regression baseline pipeline now standardizes channels, applies class weighting, and fits on a downsampled subset to validate convergence; full-scale training awaits batch-queued GPU time. U-Net data loaders with augmentation hooks (horizontal/vertical flips) were also verified offline. No quantitative metrics are reported yet because large-scale training runs are scheduled after cluster maintenance on 15 Nov 2025.
...
Robert Harris is leading data engineering, TFRecord ingestion, and baseline logistic regression experiments. Hannah Richardson is implementing and tuning the shallow CNN and U-Net models, including augmentation and mixed-precision training. Chelsey Blowe is expanding the literature survey, designing the evaluation protocol, and drafting the robustness and calibration analyses. All members will review experiment logs and contribute to documentation prior to submission.
